...
This manual documents the workflow developed to extract noncollinear Spin-Orbit Coupling (SOC) Hamiltonians from FHI-aims localized atomic orbital outputs and process them into the block-sparse format utilized by the DeepH machine learning ecosystem. It covers the necessary modifications to the Fortran source code of FHI-aims to expose the spatial SOC $\Pi$ matrices, the mathematical reconstruction and Hermiticity constraints of the full $2N \times 2N$ complex Hamiltonian, and the automated Python parsing strategies, including both a modified DeepH extraction tool and a standalone scripting route, used to validate the workflow. This package provides everything needed to reproduce, validate, and utilize the FHI-aims to DeepH SOC data pipeline.
